...
import { Search, Globe, Menu, X, ArrowRight, PhoneCall, User, ChevronDown } from 'lucide-react';
...
export default function Navbar({
  ...
  return (
    ...
    >
        ...
        <nav className="hidden lg:flex items-center gap-6 relative">
          ...
          >
            <Link
              ...
              className="text-xs font-bold uppercase tracking-wider text-slate-900 hover:text-[#00c853] transition-colors py-2 flex items-center gap-1 cursor-pointer"
            >
              <span>Products</span>
        ...
        <div className="hidden sm:flex items-center gap-4">
          ...
          {currentUser ? (
            <div className="flex items-center gap-2 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded text-xs text-slate-800">
              ...
              {currentUser.isAdmin && (
                <Link to="/admin/dashboard" className="text-[10px] font-bold text-blue-600 hover:underline">
                  CMS
                </Link>
              )}
              {onLogout && (
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-slate-200 px-6 py-6 space-y-4 shadow-xl animate-fadeIn max-h-[85vh] overflow-y-auto">
