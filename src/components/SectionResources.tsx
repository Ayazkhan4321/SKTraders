    ...
    <section id="resources" className="py-20 bg-white border-b border-slate-100">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          {/* Left Column Text & Action matching Screenshot 1 */}
          <div className="lg:col-span-5 flex flex-col items-start space-y-6 pr-0 lg:pr-8">
            ...
            <button
              onClick={onOpenQuote}
              className="btn-signify-outline text-sm font-medium border border-slate-900 hover:bg-slate-900 hover:text-white px-6 py-3 transition-all"
            >
              <span>Explore all resources</span>
            </button>
          </div>

          {/* Right Column: 2 Resource Cards matching Screenshot 1 */}
          <div className="lg:col-span-7 flex flex-col space-y-4">
            {resourceCards.map((card) => (
              <div
                key={card.id}
                onClick={onOpenQuote}
                className="group cursor-pointer bg-slate-50 hover:bg-slate-100/80 transition-all duration-300 flex flex-col sm:flex-row items-stretch overflow-hidden border border-slate-100"
              >
                {/* Image Thumbnail */}
